...
### 5.3.2 用户显式发起的连接（"附近"页点击某人）

§5.3 的自动决策决定"谁先发起"，但它不是用户意图：当用户点了一个可见的人，本机**立即发起**连接，
**不做 ticket 比较**。用户优先级高于 ticket 优化。

仍然受两条限制约束：`MAX_LINKS` 上限，以及 §5.3.4 的失败退避。两侧同时点击会产生两条链路，
由 §5.4 正常收敛到一条——这正是 §5.4 存在的原因，因此不需要为手动连接引入额外规则。

一次点击同时会被记为"用户正在等这个人的安全码"：该对端握手完成且信任状态为 UNVERIFIED 时，
本机**恰好一次**发出 `NodeEvent.VerifyRequested`（Android）/ `.verifyRequested`（iOS）供 UI 弹窗；
若该对端已是 TRUSTED，或 20 秒内没有连上，则不再发出。**自动连接绝不弹窗**，否则一个房间里
8 个对端会连续打断用户。这条 pending 在 §5.4 去重丢弃链路时会转移到存活的那条链路上
（两条链路的 peer deviceId 相同，必然是同一人）。
