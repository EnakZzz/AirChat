...
连接由后台自动完成（公共频道要发给附近所有人，所以不能等用户逐一点击），但**点谁、核对谁、
...
| 「附近」页交互（点人即连 / 核对 / 进对话） | ✅ 两端实现一致；单测覆盖点击、一次性弹窗、去重串联、上限拒绝；真机由 `--connect-first` 自动断言 |
...
都收到了核对安全码的请求。一次启动不能既发脚本消息又点人，所以完整验证要跑两次。
